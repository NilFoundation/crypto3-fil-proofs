#ifndef FILECOIN_PARAMETER_CACHE_HPP
#define FILECOIN_PARAMETER_CACHE_HPP
...
namespace filecoin {
    ...
    template<template<typename> class Circuit, typename Bls12, typename ParameterSetMetadata = parameter_set_metadata>
    struct cacheable_parameters {
        ...
        std::string cache_identifier(const P &pub_params) {
            ...
            std::string circuit_hash = hash<sha2<256>>(pub_params.identifier());
        ...
        cache_entry_metadata get_param_metadata(const C &circuit, const P &pub_params) {
            ...
            boost::filesystem::path meta_path = detail::ensure_ancestor_dirs_exist(parameter_cache_metadata_path(&id));
        ...
        groth::mapped_params<Bls12> get_groth_params(UniformRandomGenerator &r, const C &circiut, const P &pub_params) {
    ...
    cache_entry_metadata read_cached_metadata(const boost::filesystem::path &cache_entry_path) {
        with_exclusive_read_lock(cache_entry_path,
                                 [&](const boost::filesystem::path &file) { return serde_json::from_reader(file); });
    }
