...
int main(int argc, char *argv[]) {
    ...
    let matches =
        App::new ("paramfetch")
            ...
            .about(&format !(
                       "
                           Set {} to specify Groth parameter and
                           verifying key - cache directory.Defaults to '{}' ",
            ...
            .arg(Arg::with_name("retry").short("r").long("retry").help("Prompt to retry on failure"), )
            .arg(Arg::with_name("all")
                     .short("a")
                     .long("all")
                     .conflicts_with("params-for-sector-sizes")
                     .help("Download all available parameters and verifying keys"), )
            .arg(Arg::with_name("params-for-sector-sizes")
                     .short("z")
                     ...
                     .value_delimiter(",")
                     .multiple(true)
                     .help("A comma-separated list of sector sizes, in bytes, for which Groth parameters will be "
                           "downloaded"), )
            .arg(Arg::with_name("verbose").short("v").long("verbose").help("Print diagnostic information to stdout"), )
            .arg(Arg::with_name("ipget-bin")
                     .conflicts_with("ipget-version")
                     .takes_value(true)
                     .short("i")
                     .long("ipget-bin")
                     .help("Use specific ipget binary instead of looking for (or installing) one in "
                           "/var/tmp/ipget/ipget"), )
            .arg(Arg::with_name("ipget-args")
                     .takes_value(true)
                     .long("ipget-args")
                     .help("Specify additional arguments for ipget"))
            .arg(Arg::with_name("ipget-version")
...
fn fetch(matches : &ArgMatches)->Result<()> {
    let manifest = if matches.is_present("json") {
        ...
        if
            !json_path.exists() {
                bail !("JSON file '{}' does not exist", &json_path.to_str().unwrap_or(""));
            }
        ...
        serde_json::from_reader(reader).with_context(
            || {format !("JSON file '{}' did not parse correctly", &json_path.to_str().unwrap_or(""), )}) ?
    }
    else {
    ...
    if matches
        .is_present("params-for-sector-sizes") {
            let whitelisted_sector_sizes
                : HashSet<u64> =
                      values_t !(matches.values_of("params-for-sector-sizes"), u64) ?.into_iter().collect();
    ...
    if
        !matches.is_present("params-for-sector-sizes") && !matches.is_present("all") && !filenames.is_empty() {
            filenames = choose_from(&filenames, | filename | {manifest.get(filename).map(| x | x.sector_size)}) ? ;
            println !();
        }
    ...
    else {PathBuf::from(&get_ipget_bin(ipget_version))};
...
fn get_ipget_bin(version
                 : &str)
    ->String {format !("{}-{}/ipget/ipget", IPGET_PATH, version)}

/// Check if ipget is available, dowwnload it otherwise.
fn ensure_ipget(is_verbose
                : bool, version
                : &str)
    ->Result<()> {
    let ipget_bin = get_ipget_bin(version);
    if Path
        ::new (&ipget_bin).exists() {
            Ok(())
        }
    else {
...
fn download_ipget(is_verbose : bool, version : &str)->Result<()> {
    ...
    let url =
        Url::parse(&format !("https://dist.ipfs.io/ipget/{}/ipget_{}_{}-amd64.{}", version, version, os, extension)) ?
        ;
...
fn download_file(url : Url, target : impl AsRef<Path>, is_verbose : bool)->Result<()> {
    ...
    let client = Client::builder().proxy(Proxy::custom(move | url | env_proxy::for_url(&url).to_url())).build() ? ;
    let total_size = { let res = client.head(url.as_str()).send() ? ;
    if res
        .status().is_success() {
            res.headers()
...
fn fetch_parameter_file(is_verbose
                        : bool, parameter_map
                        : &ParameterMap, filename
                        : &str, ipget_bin_path
                        : impl AsRef<Path>, ipget_args
    ...
    download_file_with_ipget(&parameter_data.cid, path, is_verbose, ipget_bin_path, ipget_args, )
}

fn download_file_with_ipget(cid
                            : impl AsRef<str>, target
                            : impl AsRef<Path>, is_verbose
                            : bool, ipget_bin_path
                            : impl AsRef<Path>, ipget_args
                            : Option<impl AsRef<str>>, )
    ->Result<()> {
    ...
    ensure !(output.status.success(), "failed to download {}", target.as_ref().display());

    Ok(())
}

fn get_filenames_requiring_download(parameter_map
                                    : &ParameterMap, parameter_ids
                                    : Vec<String>, )
    ->Result<Vec<String>> {Ok(parameter_ids.into_iter()
                                  .filter(| parameter_id |
                                          {
                                              println !("checking: {}", parameter_id);
                                              print !("does file exist... ");

                                              if get_full_path_for_file_within_cache (parameter_id)
                                                  .exists() {
                                                      println !("yes");
                                                      print !("is file valid... ");
                                                      io::stdout().flush();
...
fn get_filenames_from_parameter_map(parameter_map
                                    : &ParameterMap)
    ->Result<Vec<String>> {Ok(parameter_map.iter().map(| (k, _) | k.clone()).collect())}

fn validate_parameter_file(parameter_map
                           : &ParameterMap, filename
                           : &str)
    ->Result<bool> {
    let parameter_data = parameter_map_lookup(parameter_map, filename) ? ;
...
fn invalidate_parameter_file(filename : &str)->Result<()> {
    ...
    let target_parameter_file_path = parameter_file_path.with_file_name(format !("{}-invalid-digest", filename));
